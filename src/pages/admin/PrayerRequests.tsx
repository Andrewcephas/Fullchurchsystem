...
const PrayerRequests = () => {
  ...
  const { branchId: userBranch } = useUserRole();

   const fetchRequests = async () => {
     const response = await apiService.getPrayerRequests();
   ...
   const updateStatus = async (id: string, status: string) => {
     ...
     if (response.error) { toast({ title: "Error", variant: "destructive" }); return; }
     toast({ title: `Marked as ${status}` });
     fetchRequests();
   };

   const handleCreate = async () => {
     if (!form.name || !form.request) { toast({ title: "Name and request required", variant: "destructive" }); return; }
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild>
            <Button className="bg-primary hover:bg-primary/90 text-primary-foreground"><Plus className="h-4 w-4 mr-2" />Add Request</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>Add Prayer Request</DialogTitle></DialogHeader>
            <div className="space-y-4">
              <div><Label>Name *</Label><Input value={form.name} onChange={e => setForm({...form, name: e.target.value})} /></div>
              <div><Label>Email</Label><Input type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} /></div>
              <div><Label>Request *</Label><Textarea value={form.request} onChange={e => setForm({...form, request: e.target.value})} /></div>
              <Button onClick={handleCreate} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">Submit</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
